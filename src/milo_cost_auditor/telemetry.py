...
Schema:
  ...
  invocations (tool TEXT, ts TEXT, count INTEGER, PRIMARY KEY(tool, ts))
"""
...
DEFAULT_HOME = Path.home() / ".milo-cost-auditor"
DB_NAME = "telemetry.db"
INSTALL_ID_NAME = "install-id"


def _home_dir() -> Path:
    """Where we keep state. Honors MILO_COST_AUDITOR_HOME for tests."""
    custom = os.environ.get("MILO_COST_AUDITOR_HOME")
...
def first_invocation_banner() -> Optional[str]:
    ...
    return (
        f"# Milo Cost Auditor v{__version__} — telemetry stays local; "
